...
class KinematicChain: 
    ...
    def add_eef_offset(self, offset):
        self.frames[-1].joint.T_offset[:3, 3] += offset
    ...
    def forward_kinematics(self, thetas: np.ndarray) -> np.ndarray:
        ...
        assert len(frames) == len(thetas), f"shape mismatch: got len(frame)={len(frames)} and len(thetas)={len(thetas)}"
        fk = []
    ...
    def calc_jacobian(self, curr_fk): 
        ...
        J = np.zeros((6, len(self.frames))) # Shape: (6, n) NOTE: self.frames -> dof 
        p_eff = curr_fk[-1][:3, 3]
    ...
    def compute_ik_newton_rapshon(self, current_thetas: np.ndarray, target_pos, target_ori=None, max_iter=500): 
        '''
        Args: 
        ...
        '''
        initial_thetas = current_thetas.copy()
        iterator = 1
        eps = float(1e-6)
        ...
        if target_ori is not None:
            assert target_ori.shape == (3, 3), f"Target orientation is wrong: currently {target_ori.shape}"
            current_ori = curr_fk[-1][:3, :3]
            err_ori = current_ori.T @ target_ori
            err_ori = axisangle_from_rot(err_ori)   
            assert err_ori.shape == (3,), f"Axis-angle representation is wrong: currently {err_ori.shape}"
            print("err_ori", err_ori)

            err_pos = np.concatenate([err_pos, err_ori])
        ...
        while err > eps: 
            ...
            # Joint limits 
            current_thetas[0] = np.clip(current_thetas[0], -np.pi/6, np.pi/6)
            current_thetas[1] = np.clip(current_thetas[1], 0, np.pi)
            current_thetas[2] = np.clip(current_thetas[2], -0.92 * np.pi, 0) # -165 ~ 0
            current_thetas[3] = np.clip(current_thetas[3], -0.44 * np.pi, 0.44 * np.pi) # -80 ~ 80
            current_thetas[4] = np.clip(current_thetas[4], -0.472 * np.pi, 0.472 * np.pi) # -85 ~ 85
            current_thetas[5] = np.clip(current_thetas[5], -0.888 * np.pi, 0.888 * np.pi) # -160 ~ 160

        
        if iterator > max_iter: 
            print("Max iteration reached")
            err_norm = np.linalg.norm(current_thetas - initial_thetas)
            current_thetas = initial_thetas # don't do anything
            info = {
                "error": "IK failed",
                "err_norm": err_norm
                }
            ...
            if target_ori is not None:
                info["target_ori"] = target_ori
                info["current_ori"] = current_ori
        
            # constrain thetas [0, 2*pi] 
            current_thetas = np.mod(current_thetas, 2*np.pi)
